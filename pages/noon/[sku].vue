...
const { checkingWoocommerce, agreed, dxbUrl, uploadingToWoocommerce } = storeToRefs( noonStore );
const { checkWoocommerce, toggleAgree } = noonStore;
...
const resp = await noonService.view(sku, 'noon');
agreed.value = false
        ...
        <div class="grid grid-cols-1 items-start gap-8 md:grid-cols-2">
          <div class="grid grid-cols-2 gap-4 md:grid-cols-1">
            <div class="grid grid-cols-4">
              <img :src="session.images[0]" class="col-start-1 col-span-3 w-full rounded-md object-cover" />
            </div>

            <div class="grid grid-cols-2 gap-4">
              <img v-for="img in session.images" :src="img" class="aspect-square w-full rounded-md object-cover" />
            </div>
          </div>
          ...
          <div class="sticky top-[190px]">
            <div class="mt-8 flex justify-between">
              <div class="max-w-full space-y-2">
                ...
                <!-- <p
                  class="text-xl font-bold text-pink-800"
                ...
                <div class="flex my-8">
                  <div class="mr-4">
                      <span class="font-bold text-gray-700 dark:text-gray-700">Price: </span>
                      <span class="text-gray-600 dark:text-green-800 font-bold">
                        {{ session.price }}
                      </span>
                  </div>
                  <div>
                      <span class="font-bold text-gray-700 dark:text-gray-700">Weight: </span>
                      <span class="text-gray-800 dark:text-gray-800"> {{ session.item_weight}}kg</span>
                  </div>
              </div>
              </div>
            </div>
            ...
            <!-- <div class="mt-4">
              <div
                ...
                :class="{ 'text-gray-100': darkMode }"
              >
                <p v-html="session.highlights" id="highlights_section">
                </p>
              </div>
            </div> -->
...
-->
              ...
              <div class="overflow-x-none">
                <div class="divide-gray-200 text-sm">
                  <div class="divide-gray-200 text-sm">
                    <div v-for="(value, key) in session.specifications" class="odd:bg-gray-50 flex justify-between" :class="{
                        'bg-gray-900': darkMode,
                        'odd:bg-gray-800': darkMode,
                      }">
                      <span class="px-4 py-2 text-gray-700 font-bold" :class="{ 'text-slate-400': darkMode }">
                        {{ key }}
                      </span>
                      <span class="px-4 py-2 text-right text-gray-700 text-clip" :class="{ 'text-slate-400': darkMode }">
                        {{ value }}
                      </span>
                    </div>
                  </div>
